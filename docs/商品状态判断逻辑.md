...
```
    ...
    └─────────┬───────────┘               └─────────┬───────────┘
       ...
       │ 是          │ 否                    │ 是          │ 否
       ▼             ▼                       ▼             ▼
   refunded      active              ┌──────────────┐  原有逻辑
   (已退款)      (已生效)             │ restock_type │  (送达/发货状态)
                                     └──────┬───────┘
                                            │
                              ┌─────────────┼─────────────┐
                              │             │             │
...
|------|-----------------|------|
| 订单全额退款 OR 所有实物商品都退款 | `refunded` | 已退款 |
| 订单已支付 且 有未退款实物商品 | `active` | 已生效 |
| 订单待支付 | `pending` | 待支付 |

**核心逻辑**：服务类商品跟随订单整体状态，只有所有实物商品都退款了，服务类商品才显示退款。

---

## 五、Shopify refunds 数据结构
...
| 商品 | 类型 | 判断逻辑 | delivery_status |
|------|------|----------|-----------------|
| Worry-Free Purchase | 服务类 | 有未退款实物(phone holder) → 已生效 | `active` |
| Fiido phone holder | 实物 | 未在refunds中 + 送达成功 | `success` |
...
| 商品 | 类型 | 判断逻辑 | delivery_status |
|------|------|----------|-----------------|
| Worry-Free Purchase | 服务类 | 所有实物都退款 → 已退款 | `refunded` |
| Rear Pegs for T2 | 实物 | 在refunds中 + restock_type=return | `returned` |
...
| 商品 | 类型 | 判断逻辑 | delivery_status |
|------|------|----------|-----------------|
| 电动车 | 实物 | 在refunds中 + restock_type=cancel | `cancelled` |
...
| 商品 | 类型 | 判断逻辑 | delivery_status |
|------|------|----------|-----------------|
| Worry-Free Purchase | 服务类 | 订单已支付 → 已生效 | `active` |
| Titan Ebike | 实物 | 送达成功 | `success` |
| Pannier Bag | 实物 | 送达成功 | `success` |
