...
## Phase 5 新增需求（2025-12-23）

### 背景

基于 v1.0 已完成的功能（Phase 1-4），用户测试反馈：
- 点击「查看物流」按钮显示"暂无物流信息"，因为运单未在 17track 注册
- 前端错误提示不够友好，用户不知道何时刷新
- SMTP 邮件配置未提供说明文档

### 根本原因分析

**问题**：17track 要求先注册运单才能查询，当前流程缺少自动注册环节。

**当前流程**：
```
前端点击 → 调用 /api/tracking/{number} → 17track 返回"未注册" → 显示"暂无物流信息"
```
...
| 功能 | 优先级 | 模块 | 说明 |
|------|--------|------|------|
| 运单自动注册 | P0 | services/tracking | 查询失败时后台异步注册 |
| 承运商自动识别 | P1 | ai_chatbot | 从 Shopify fulfillment 提取承运商 |
| 前端错误优化 | P1 | ai_chatbot/frontend | 区分不同状态，优化提示信息 |
| SMTP 配置文档 | P1 | 文档 | 提供邮件服务配置说明 |

### 最优解：立即返回 + 后台异步注册

用户体验优先，不让用户等待：
1. 查询时如果未注册 → 后台异步注册（不阻塞）
2. 立即返回状态"tracking_pending"（追踪中）
3. 前端显示"物流信息更新中，请稍后刷新"
4. 用户刷新页面时数据已就绪

### 新增接口

无新增 API，仅修改现有接口行为：
- `GET /api/tracking/{tracking_number}` - 增加自动注册逻辑

### 成功标准

- [ ] 用户点击查看物流，首次显示"追踪中"提示
- [ ] 刷新后显示完整物流轨迹
- [ ] 前端错误信息清晰友好
- [ ] SMTP 配置文档完整可用
