...
3. 【新增 v1.2】17track 缓存状态（Shopify 无数据时补充）
   └── 当 shipment_status 为空但有 tracking_number 时
   └── 查询 17track 缓存获取真实物流状态
   └── DELIVERED → 已收货 (success)
   └── IN_TRANSIT → 运输中 (in_transit)
   └── OUT_FOR_DELIVERY → 派送中 (out_for_delivery)
   └── ALERT/UNDELIVERED → 投递失败 (failure)

4. 发货状态（最低优先级）- 当 shipment_status 和 17track 都无数据时
   └── fulfillment_status = fulfilled → 已发货
   └── fulfillment_status = null → 待发货
```

**代码逻辑**（`services/shopify/client.py`）：
```python
# 状态判断优先级：shipment_status > 17track缓存 > fulfillment_status
...
if shipment_status:
    ...
    # 发货成功但 Shopify 没有 shipment_status
    # 【v1.2 新增】尝试从 17track 缓存获取真实状态
    ...
    if tracking_number and track17_service:
        ...
        # ... 其他状态映射
            ...
            delivery_status = None  # 无缓存，显示"已发货"
    else:
...
**适用场景**：
- Shopify `shipment_status` 为 null（旧订单不会自动更新）
- 但 17track 有该运单的物流数据（用户之前点击过"查看物流"）

**实现原理**：
1. 用户点击"查看物流"时，系统查询 17track API 并缓存结果（6 小时 TTL）
2. 下次查询订单时，检查 17track 缓存是否有该运单的状态
3. 如果 17track 显示 Delivered，则设置 `delivery_status = "success"`

**性能说明**：
- 只读缓存，不发起 17track API 请求
- 执行时间 < 1ms（内存缓存）

**相关代码**：
- `services/tracking/service.py` - `get_cached_status_sync()` 方法
- `services/shopify/client.py` - `_parse_order_detail_impl()` 方法
...
### v1.2 (2025-12-24)
- **新增 17track 缓存补充**：当 Shopify `shipment_status` 为空时，使用 17track 缓存数据补充真实物流状态
- 解决旧订单 Shopify 不更新 `shipment_status` 导致显示"已发货"而非"已收货"的问题
- 新增 `get_cached_status_sync()` 同步缓存查询方法
- 更新状态优先级：shipment_status > 17track缓存 > fulfillment_status
