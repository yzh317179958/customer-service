...
- 变体格式（全部有效）：
  - 纯订单号：UK22080、NL16479、DE10090
  - 带 # 前缀：#UK22080、#NL16479、#DE10090
  - 带 - 分隔：UK-22080、NL-16479
  - 小写形式：uk22080、nl16479
- **提取规则**：去掉 # 和 - 符号，转为大写
- **重要**：只要消息中包含上述格式的订单号，即使只有订单号本身，也应识别为 tracking 意图

**示例**：
- "帮我查一下订单 UK22080" → tracking, order_number: UK22080
- "NL16479 发货了吗" → tracking, order_number: NL16479
- "Check order DE10090" → tracking, order_number: DE10090
- "#NL16479" → tracking, order_number: NL16479（注意：即使只有带#的订单号也要识别！）
- "#UK22080" → tracking, order_number: UK22080
- "order #DE10090" → tracking, order_number: DE10090
...
- **输入变体**：
  - `#NL16479` → 提取为 `NL16479`
  - `#UK22080` → 提取为 `UK22080`
  - `UK-22080` → 提取为 `UK22080`
  - `uk22080` → 提取为 `UK22080`
- **提取规则**：去掉 # 和 - 符号，转为大写
- **示例输出**：UK22080、NL16479、DE10090
...
{
  "intent": "tracking",
  "order_number": "NL16479",
  "email": null,
  "confidence": 0.95,
  "reason": "用户提供了带#前缀的订单号#NL16479，提取为NL16479",
  "from_intent_param": false
}
...
**输出**：
{
  ...
  "email": null,
  "confidence": 0.95,
  "reason": "用户查询订单，提取订单号UK22080（去掉#前缀）",
  "from_intent_param": false
}
...
### v3.2 (2025-12-30)
- **修复带#前缀订单号识别问题**：
  - 明确 `#NL16479`、`#UK22080` 等带#的订单号格式
  - 新增示例5和示例6专门演示带#订单号的识别
  - 增强实体提取规则，明确输入→输出的转换示例
- **强调**：即使只有带#的订单号（如`#NL16479`），也必须识别为 tracking 意图
