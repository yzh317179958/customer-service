...
```mermaid
graph LR
    A[bot_active] -->|触发监管/用户请求| B[pending_manual]
    B -->|坐席接入| C[manual_live]
    C -->|坐席释放| A
    B -->|非工作时间| D[after_hours_email]
    D -->|坐席补回| C
    C -->|超时/关闭| E[closed]
    E -->|重新激活| A
```
...
| 默认账号初始化 | ✅ 已完成 | 3个默认账号          |
| **P0 管理员功能** | | |
| JWT 权限中间件 | ✅ 已完成 | require_admin(), require_agent() |
| 坐席账号管理   | ✅ 已完成 | CRUD 操作（列表、创建、修改、删除） |
| 角色权限控制   | ✅ 已完成 | admin/agent 角色区分 |
| 密码重置       | ✅ 已完成 | 管理员重置坐席密码   |
| **P1 自助功能** | | |
| 修改自己密码   | ✅ 已完成 | 旧密码验证 + 新密码强度检查 ⭐ v3.1.2 |
| 修改个人资料   | ✅ 已完成 | 只允许修改 name 和 avatar_url ⭐ v3.1.3 |
