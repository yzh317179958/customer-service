...
>
> **本技能是 memory-bank-guide 的跨模块扩展版**，核心流程完全一致，仅在文件位置和内容上适配跨模块场景。
...
> 与单模块 memory-bank 完全一致，仅位置不同

```
docs/features/[功能名]/
├── prd.md                 # 产品需求文档（为什么做、做什么）
├── tech-stack.md          # 技术栈说明（用什么技术、跨模块通信方案）
...
**输出格式（参考模板 docs/features/_templates/prd.md）：**
...
**输出格式（参考模板 docs/features/_templates/implementation-plan.md）：**
...
> 如果该文件不存在，则创建它（参考模板 `docs/features/_templates/cross-module-refs.md`）
...
## 创建新跨模块功能后的后续步骤（来自 CLAUDE.md）

1. **更新 PROJECT_OVERVIEW.md**（如涉及新模块）
2. **更新 .env 配置**（如需要环境变量）
