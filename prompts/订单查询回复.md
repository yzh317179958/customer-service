...
**⚠️ 重要：必须同时检查 `financial_status` 和 `fulfillment_status` 两个字段！**

**支付状态 (financial_status) - 优先级最高**：
| 原始值 | 英文显示 | 中文显示 | 订单状态显示 | 商品卡片状态 |
|-------|---------|---------|-------------|-------------|
| paid | - | - | 显示发货状态 | 显示发货状态 |
| pending | Payment Pending | 待支付 | ⚠️ Payment Pending / 待支付 | Pending / 待支付 |
| refunded | Refunded | 已退款 | ⚠️ Refunded / 已退款 | Refunded / 已退款 |
| partially_refunded | Partially Refunded | 部分退款 | ⚠️ Partially Refunded / 部分退款 | 显示发货状态 |

**发货状态 (fulfillment_status) - 仅当 financial_status = paid 时使用**：
| 原始值 | 英文显示 | 中文显示 |
|-------|---------|---------|
...
2. 如果是 `refunded` → 订单状态显示"已退款"，商品卡片状态也显示"Refunded/已退款"
3. 如果是 `pending` → 订单状态显示"待支付"，商品卡片状态显示"Pending/待支付"
4. 如果是 `paid` → 根据 `fulfillment_status` 显示发货状态
...
| 位置 | 字段 | 必填 | 说明 |
|-----|------|-----|------|
| 1 | 图片URL | 否 | 商品图片，无图片留空 |
| 2 | 商品名称 | 是 | 完整商品名称 |
| 3 | 数量 | 是 | 购买数量 |
| 4 | 价格 | 是 | 带货币符号，如 £1,499.00 |
| 5 | 状态 | 是 | **根据 financial_status 和 fulfillment_status 判断**：<br>- financial_status=refunded → Refunded/已退款<br>- financial_status=pending → Pending/待支付<br>- financial_status=paid + fulfilled → Shipped/已发货<br>- financial_status=paid + null → Pending/待发货 |
| 6 | 承运商 | 否 | 如 DX FREIGHT、DHL，未发货或已退款留空 |
| 7 | 运单号 | 否 | 如 G9375063，未发货或已退款留空 |
| 8 | 追踪链接 | 否 | 完整URL，未发货或已退款留空 |
...
- **状态显示**：
  - 正常订单：已发货/Shipped 或 待发货/Pending
  - 退款订单：已退款/Refunded
  - 待支付订单：待支付/Pending
